...
public class PollRemovePlugin implements Runnable { 
	...
	public void run() {
        try {
            ...
            if(controllerEngine.getGDB().dba.getINodeStatus(inode_id) > 8) {
            String edge_id = controllerEngine.getGDB().dba.getResourceEdgeId(resource_id, inode_id);
            if (edge_id != null) {
                String pnode_node_id = controllerEngine.getGDB().dba.getIsAssignedParam(edge_id, "out");
                if (pnode_node_id != null) {
                    ...
                    String pnode_node_id_match = controllerEngine.getGDB().gdb.getNodeId(region, agent, pluginId);
                    ...
                    if (pnode_node_id.equals(pnode_node_id_match)) {
                        ...
                        if(re != null) {
                            ...
                            if(statusCode == 7) {
                                boolean isRemovedPlugin = controllerEngine.getGDB().gdb.removeNode(region,agent,pluginId);
                            }
                        } else {
                        ...
                        while ((!isRemoved) && (count < 60)) {
                            if (controllerEngine.getGDB().gdb.getNodeId(region, agent, pluginId) == null) {
                                isRemoved = true;
                            }
                            Thread.sleep(1000);
                            count++;
                        ...
                        if (isRemoved) {
                            controllerEngine.getGDB().dba.setINodeParam(inode_id, "status_code", "8");
                            controllerEngine.getGDB().dba.setINodeParam(inode_id,"status_desc","iNode Disabled");
                            logger.debug("removed r: " + region + " a:" + agent + " p:" + pluginId + " for inode:" + inode_id);
                        } else {
                            controllerEngine.getGDB().dba.setINodeParam(inode_id, "status_code", "90");
                            controllerEngine.getGDB().dba.setINodeParam(inode_id,"status_desc","iNode unable to verify iNode deactivation!");
                            logger.debug("pollRemovePlugin : unable to verify iNode deactivation! " + controllerEngine.getGDB().dba.getINodeStatus(inode_id));
                        ...
                        controllerEngine.getGDB().dba.setINodeParam(inode_id, "status_code", "91");
                        controllerEngine.getGDB().dba.setINodeParam(inode_id,"status_desc",errorString);
                    }
